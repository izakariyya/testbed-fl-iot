# -*- coding: utf-8 -*-
"""
Created on Thu Dec 30 14:26:07 2021
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(4 * 4 * 50, 128)#nn.Linear(4 * 4 * 50, 500)
        self.fc2 = nn.Linear(128, 10)#nn.Linear(128, 10)
...
def train_on_e_batches(worker, batches, model_in, device, lr):
    ...
    for batch_idx, (data, target) in enumerate(batches):
        ...
        #optimizer.zero_grad()   
        output = model(data)
        #loss_o = f.nll_loss(output, target)
        loss = f.nll_loss(output, target)
        #loss = lambd * loss_o   
